...
export function renderMarkdownReport({
	...
	if (groups.typeMismatch.length) {
		...
		for (const d of groups.typeMismatch) {
			lines.push(
				`- **${d.path || '(root)'}**: DB type \`${(d as any).dbType}\` vs File type \`${(d as any).fileType}\``
			);
			lines.push('');
			lines.push('```json');
...
export async function runCompare(rawArgv: string[]): Promise<void> {
	...
	const unorderedList = Array.isArray(unorderedArg)
		? unorderedArg
		: unorderedArg
			? [unorderedArg]
			: [];
	...
	if (((args['fail-on-diff'] as boolean) || (args.fail as boolean)) && diffs.length > 0)
		process.exit(2);
}
